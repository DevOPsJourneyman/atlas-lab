...
Deploy a robust, networked storage solution to handle automated backups for a Proxmox VE Cluster. 
#### Solution: 
Implemented TrueNAS Scale as a dedicated NFS storage backend, configured automated VM snapshot schedules, and established a disaster recovery workflow.
...
* Set Maproot User and Maproot Group to ensure Proxmox has root-level write access to the specific share.

##### Security: 
* Whitelisted the IP addresses of the Proxmox nodes to restrict access.
