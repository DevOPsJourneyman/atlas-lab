...
Create sdn zone
    ...
    add ip addresses
    mtu: 1450
    nodes: atlas01 + atlas02
Create Vnet inside the sdn
    Name: vnet10
    tag: 10
...
### Test > Does atlas VM ping correctly? 
    ...
    lan gateway check
    access laptop check
    Repeat test on atlas02 vm
    change nic to vnet10
    update the netplan to DCHP
    sudo netplan generate
    sudo netplan apply
    correct DHCP IP Assigned
    ping to gateway, pfsense and laptop Success
...
### Solution
    ...
    ping is working in both directions now. 